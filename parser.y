...
simple_stmt:
        T_NAME '=' expr {
            $$ = node_new(c->arena, NODE_ASSIGN, $1, $3);
        }
    |   name_list_morethanone '=' non_empty_expr_list_inline {
            $$ = node_new(c->arena, NODE_ASSIGN_MULTI, $1, $3);
        }
    |   T_LOCAL T_NAME '=' expr {
            $$ = node_new(c->arena, NODE_ASSIGN_LOCAL, $2, $4);
        }
    |   expr '[' expr ']' '=' expr {
            $$ = node_new(c->arena, NODE_STORE_SUBSCRIPT, NULL, NULL);
            $$->nd_left = $1;
            $$->nd_middle = $3;
            $$->nd_right = $6;
        }
    |   expr '.' T_NAME '=' expr {
            $$ = node_new(c->arena, NODE_STORE_DOTSUBSCRIPT, NULL, NULL);
            $$->nd_left = $1;
            $$->nd_middle = $3;
            $$->nd_right = $5;
        }
    |   T_NAME T_ADD_ASSIGN expr {
            Node *t = node_new(c->arena, NODE_BIN, $1, $3);
            t->u.op = OP_BINARY_ADD;
            $$ = node_new(c->arena, NODE_ASSIGN, $1, t);
        }
    |   T_NAME T_SUB_ASSIGN expr  {
            Node *t = node_new(c->arena, NODE_BIN, $1, $3);
            t->u.op = OP_BINARY_SUB;
            $$ = node_new(c->arena, NODE_ASSIGN, $1, t);
        }
    |   T_NAME T_MUL_ASSIGN expr  {
            Node *t = node_new(c->arena, NODE_BIN, $1, $3);
            t->u.op = OP_BINARY_MUL;
            $$ = node_new(c->arena, NODE_ASSIGN, $1, t);
        }
    |   T_NAME T_DIV_ASSIGN expr  {
            Node *t = node_new(c->arena, NODE_BIN, $1, $3);
            t->u.op = OP_BINARY_DIV;
            $$ = node_new(c->arena, NODE_ASSIGN, $1, t);
        }
    |   T_NAME T_MOD_ASSIGN expr {
            Node *t = node_new(c->arena, NODE_BIN, $1, $3);
            t->u.op = OP_BINARY_MOD;
            $$ = node_new(c->arena, NODE_ASSIGN, $1, t);
        }
    ...
    |   T_BREAK {
            $$ = node_new(c->arena, NODE_BREAK, 0, 0);
        }
    |   T_CONTINUE {
            $$ = node_new(c->arena, NODE_CONTINUE, 0, 0);
        }
    |   T_THROW {
            $$ = node_new(c->arena, NODE_THROW, 0, 0);
        }
    |   T_THROW expr {
            $$ = node_new(c->arena, NODE_THROW, $2, 0);
        }
    |   expr '(' expr_list ')' {
    ...
    |   T_IMPORT non_empty_nameoralias_list {
            $$ = node_new(c->arena, NODE_IMPORT, NULL, NULL);
            $$->nd_fromname = 0;
            $$->nd_importlist = $2;
        }
    |   T_FROM T_NAME T_IMPORT non_empty_nameoralias_list {
            $$ = node_new(c->arena, NODE_IMPORT, NULL, NULL);
            $$->nd_fromname = $2;
            $$->nd_importlist = $4;
        }
    |   T_FROM T_NAME T_IMPORT '*' {
            $$ = node_new(c->arena, NODE_IMPORT, NULL, NULL);
            $$->nd_fromname = $2;
            $$->nd_importlist = (void *)-1; /* indicates start */
        }
