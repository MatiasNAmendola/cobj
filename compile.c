...
{
    uint next_op_num = opline_array->last++;
...
{
    uint if_cond_opline_num = CG(active_opline_array)->last;
...
{
    uint if_after_stmt_op_num = CG(active_opline_array)->last;
    struct co_opline *opline = get_next_op(CG(active_opline_array));
...
{
    uint if_end_op_num = CG(active_opline_array)->last;
    CG(active_opline_array)->ops[closing_bracket_token->u.opline_num].op1.u.opline_num = if_end_op_num;
}
...
{
    uint while_cond_opline_num = CG(active_opline_array)->last;
    struct co_opline *opline = get_next_op(CG(active_opline_array));
    opline->opcode = OP_JMPZ;
    opline->op1 = *cond;
...
co_while_end(const struct cnode *while_token, const struct cnode *closing_bracket_token)
{
    ...
    uint while_end_stmt_op_num = CG(active_opline_array)->last;
    CG(active_opline_array)->ops[closing_bracket_token->u.opline_num].op2.u.opline_num =
...
{
    uint function_opline_num = CG(active_opline_array)->last;
    struct co_opline *op = get_next_op(CG(active_opline_array));
...
co_end_function_declaration(const struct cnode *function_token)
{
    ...
    uint function_end_opline_num = CG(active_opline_array)->last;
