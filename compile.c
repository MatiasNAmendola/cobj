...
compiler_is_upval(struct compiler *c, COObject *o)
{
    ...
    if (!CODict_GetItem(c->u->u_names, o)
        ...
        while (i > 0) {
            ...
            if (CODict_GetItem(tmp->u_localnames, o)) {
                is_upval = true;
                break;
            }
            i--;
...
compiler_visit_node(struct compiler *c, Node *n)
{
    ...
    case NODE_TRY:
        {
            struct block *body = NULL, *end = NULL, *handler = NULL, *orelse =
                NULL, *finally_end = NULL;
            ...
            if ((intptr_t) n->nd_importlist != -1) {
                ...
                while (l) {
                    ...
                    if (l->nd_node->nd_alias) {
                        oparg =
                            compiler_add(c->u->u_names,
                                         l->nd_node->nd_alias->u.o);
                    }
                    compiler_addop_i(c, OP_STORE_NAME, oparg);
                    l = l->nd_next;
                }
