...
{
    stack->top = 0;
    stack->elements = (void **)COMem_MALLOC(sizeof(void *) * STACK_BLOCK_SIZE);
    if (!stack->elements) {
        return false;
    } else {
...
co_stack_push(co_stack *stack, const void *element, int size)
{
    if (stack->top >= stack->max) {     /* we need to allocate more memory */
        stack->max += STACK_BLOCK_SIZE;
        stack->elements =
            (void **)COMem_REALLOC(stack->elements, sizeof(void *) * stack->max);
        if (!stack->elements) {
            return false;
        }
    }

    stack->elements[stack->top] = (void *)COMem_MALLOC(size);
